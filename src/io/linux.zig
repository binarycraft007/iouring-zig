    ...
    pub fn init(entries: u12, flags: u32) !IO {
        ...
        const index = blk: {
            if (std.mem.count(u8, release, ".") > 2) {
                break :blk std.mem.lastIndexOf(u8, release, ".").?;
            } else {
                break :blk release.len;
            }
        };
        const version = try std.SemanticVersion.parse(release[0..index]);
    ...
    pub const Completion = struct {
        ...
        fn complete(completion: *Completion) void {
            switch (completion.operation) {
                ...
                .timeout => {
                    assert(completion.result < 0);
                    const result = switch (@as(os.E, @enumFromInt(-completion.result))) {
                        .INTR => {
                            completion.io.enqueue(completion);
                            return;
                        },
